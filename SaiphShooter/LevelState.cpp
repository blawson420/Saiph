...
namespace	// Global to this file only
{
	...
	ProgressBar *CreateProgressBar() {
		...
		progressBar->SetTexture("Resources/images/progressBar2.png");
		progressBar->SetDimensions(32.0f, 8.0f);
		return progressBar;
	}
	...
	BaseObject *CreateBackground() {
		...
		background->SetPosition(BOUNDS_SIZE / 1.17F, BOUNDS_SIZE / 1.17F);
		ViewManager::GetInstance().AddObject(background, 0);
		return background;
	}
		
	//Create Power Up
	PowerUp *CreatePowerUp(int id, Vec2f position, Vec2f velocity) {
		PowerUp *powerUp = new PowerUp(id);
		powerUp->SetDimensions(32, 32);
		powerUp->SetPosition(position);
		powerUp->SetVelocity(velocity);
...
void LevelState::CheckCollision()
{
	...
	for(unsigned cur=0;cur<enemies.size();cur++)
	{
		if(enemies[cur]->GetShip() && enemies[cur]->GetShip()->GetActive())
		{
			...
			if (playerShip->Collide(enemies[cur]->GetShip()))
			{
			...
			{
				if (!enemies[cur]->GetShip()->GetActive()) break; // enemy is already dead from previous bullet collision
			...
			if(playerShip)
			{
				if(enemies[cur]->GetShip()->GetActive() && playerShip->Collide(enemies[cur]->GetShip()))
				{
...
void LevelState::GeneratePowerUps(Enemy *_enemy)
{
	...
	for(unsigned int i = 0; i < _enemy->GetPowerUpAmount(); ++i)
	{
		...
		Vec2f position = _enemy->GetShip()->GetPosition();
		powerUps.push_back(CreatePowerUp(_enemy->GetPowerUpID(), position, randomVelocity));
	}
}
...
void LevelState::ResetPlayer()
{
	...
	MotherShip* playerShip = new MotherShip(*deleteme);
	ViewManager::GetInstance().RemoveObject(deleteme);
	delete deleteme;
	ViewManager::GetInstance().AddObject(playerShip, 2);
	//playerShip = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID());
	playerShip->SetDeflector(new Deflector(playerShip, this));
	playerShip->SetShield(SaiphApp::GetShieldFlyweight(SaiphApp::GetShieldID()).strength);
	playerShip->SetBattery(SaiphApp::GetGeneratorFlyweight(SaiphApp::GetGeneratorID()).battery);
	playerShip->SetPosition(BOUNDS_SIZE / 2.0f, BOUNDS_SIZE / 2.0f);
	playerShip->SetAfterburner(SaiphApp::GetPropulsionFlyweight(SaiphApp::GetPropulsionID()).afterburnerEnergy);
	for (int i = 0; i < 5; i++)
	{
		Ship* shadow = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID(), 1);
		shadow->SetColor(1, 0.5, 0.5, 0.5);
		playerShip->addShadow(shadow);
	}
...
unsigned int LevelState::CheckForInput()
{
	...
	{
		if (playerShip->GetDeflector()->GetActive())
	...
	{
		//Activate the DeflectionStrategyA for the player ship in the Deflector
		playerShip->GetDeflector()->ActivateStrategy("DeflectionStrategyA.dll");
	}

	if (GetAsyncKeyState('G') & 0x1)
	{
		//Activate the DeflectionStrategyA for the player ship in the Deflector
		playerShip->GetDeflector()->ActivateStrategy("DeflectionStrategyB.dll");
	}
	if(!gamePaused)
	{
...
void LevelState::RenderFrame() const
{
	...
	if(gamePaused)
	{
		...
		else if(playerShip && !playerShip->GetActive())
